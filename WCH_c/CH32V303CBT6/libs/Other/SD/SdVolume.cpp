...
uint8_t SdVolume::cacheRawBlock(uint32_t blockNumber, uint8_t action)
{
	if (cacheBlockNumber_ != blockNumber) {
		if (!cacheFlush()) {
#ifdef DEBUG_SD
			printf("cacheRawBlock ! cacheFlush\r\n");
#endif
			return false;
		}
		if (!sdCard_->readBlock(blockNumber, cacheBuffer_.data)) {
#ifdef DEBUG_SD
			printf("cacheRawBlock ! sdCard_->readBlock\r\n");
#endif
			return false;
		}
...
uint8_t SdVolume::chainSize(uint32_t cluster, uint32_t* size)
const {
	uint32_t s = 0;
	do {
...
#ifdef DEBUG_SD
			printf("Error ! cacheRawBlock (part)\r\n");
#endif
			return false;
		}
...
#ifdef DEBUG_SD
			printf("Error not a valid partition\r\n");
#endif
			return false;
		}
...
#ifdef DEBUG_SD
		printf("Error not valid FAT volume\r\n");
#endif
		return false;
	}
...
#ifdef DEBUG_SD
			printf("Error clusterSizeShift_++ > 7\r\n");
#endif
			return false;
		}
	}
