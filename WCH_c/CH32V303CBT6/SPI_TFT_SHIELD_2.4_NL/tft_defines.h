...
#ifndef __TFT_DEFINES_H_
#define __TFT_DEFINES_H_
...
#define PIN_LOW(port, pin)    (port)->BCR |= (1 << pin)
#define PIN_HIGH(port, pin)   (port)->BSHR |= (1 << pin)
#define PIN_OUTPUT(port, pin) (port)->CFGLR &= ~(GPIO_Msk << pin*4); (port)->CFGLR |= (GPIO_Speed_50 << pin*4)

/*
PB8 - 0; PB9 - 1;  PB12 - 2; PB15 - 3;
PB4 - 4; PB13 - 5; PB6 - 6;  PB7 - 7;  
*/
...
// 3
#define write_8(x) { \
		GPIOB->BSHR |= (  (x) & 0b11010000); \
		GPIOB->BSHR |= ( ((x) & 0b00100011) << 8); \
		GPIOB->BSHR |= ( ((x) & 0b00000100) << 10); \
		GPIOB->BSHR |= ( ((x) & 0b00001000) << 12); \
	}

#define read_8() { \
(uint8_t)((GPIOB->INDR &         0b11010000) | \
		 ((GPIOB->INDR &   0b10001100000000) >> 8) | \
		 ((GPIOB->INDR &    0b1000000000000) >> 10) | \
		 ((GPIOB->INDR & 0b1000000000000000) >> 12)) \
	}
...
#define setReadDir()  { \
    GPIOB->CFGLR &= ~(GPIO_Msk << PB4*4); \
    GPIOB->CFGLR |= (GPIO_IPU << PB4*4); \
    GPIOB->CFGLR &= ~(GPIO_Msk << PB6*4); \
    GPIOB->CFGLR |= (GPIO_IPU << PB6*4); \
    GPIOB->CFGLR &= ~(GPIO_Msk << PB7*4); \
    GPIOB->CFGLR |= (GPIO_IPU << PB7*4); \
    GPIOB->CFGHR &= ~(GPIO_Msk << (PB8-8)*4); \
    GPIOB->CFGHR |= (GPIO_IPU << (PB8-8)*4); \
    GPIOB->CFGHR &= ~(GPIO_Msk << (PB9-8)*4); \
    GPIOB->CFGHR |= (GPIO_IPU << (PB9-8)*4); \
    GPIOB->CFGHR &= ~(GPIO_Msk << (PB12-8)*4); \
    GPIOB->CFGHR |= (GPIO_IPU << (PB12-8)*4); \
    GPIOB->CFGHR &= ~(GPIO_Msk << (PB13-8)*4); \
    GPIOB->CFGHR |= (GPIO_IPU << (PB13-8)*4); \
    GPIOB->CFGHR &= ~(GPIO_Msk << (PB15-8)*4); \
    GPIOB->CFGHR |= (GPIO_IPU << (PB15-8)*4); \
}

#define write8(x)     { write_8(x); WR_STROBE; }
#define write16(x)    { uint8_t h = (x)>>8, l = (uint8_t)x; write8(h); write8(l); }
